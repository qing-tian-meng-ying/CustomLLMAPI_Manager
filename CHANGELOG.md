...
- **Key 一键复制** - Keys 页面新增 API Key 复制按钮，点击后按需获取完整 Key（列表接口保持脱敏）
- **日志详情接口** - 新增 `GET /api/logs/[id]`，日志列表瘦身、详情按需加载

### ⚡ 性能优化

- **数据库覆盖索引** - 日志表新增三组覆盖索引，统计 / 模型路由 / token 监控接口从 0.7~1s 降到 20~100ms
- **统计聚合下推 SQL** - `/api/stats` 改为 SQL 内聚合，输出与旧算法逐字段等价
- **日志列表瘦身** - 列表不再返回 request/response body（5 条 418KB → 3.6KB），改为服务端摘要

### 🐛 修复

- **`.gitignore` 误伤 `logs/`** - `logs/` 规则曾导致 `src/app/logs/` 与 `src/app/api/logs/` 整个目录不进版本控制，已改为只忽略根目录
- **日志状态点** - 成功请求不再显示状态点，失败请求显示黄/红点，修正 3xx 误报为红色
- **README 维护 SQL** - `created_at` 以秒存储，清理/性能分析示例中的 `* 1000` 已修正

### 🧰 工程

- `dev.bat` 纳入版本控制（固定 Node 22 的启动器），`.gitignore` 忽略 PowerShell 生成的 `Microsoft/` 缓存
